...
class SamlService {
  ...
  protected function doLogin($unique_id, AccountInterface $account = NULL) {
    ...
    if (!$account) {
      $entity = \Drupal::service('entity.repository')->loadEntityByUuid('user', $unique_id);

      if ($entity && $entity->id() === $this->currentUser->id()) {
        $is_valid = (int) $entity->get('field_email_is_valid')->getValue()['value'];

        if ($is_valid == 0 && isset($attributes['hetu'])) {
          $entity->set('field_email_is_valid', 1);
          $attributes = $this->getAttributes();

          if (is_array($attributes['hetu'])) {
            $pid = reset($attributes['hetu']);
          }
          else {
            $pid = $attributes['hetu'];
          }
          ...
          $hetuhash = Crypt::hmacBase64($pid, $hash_key);
          $entity->set('field_saml_hash', $hetuhash);
          $entity->save();
          $account = $entity;
