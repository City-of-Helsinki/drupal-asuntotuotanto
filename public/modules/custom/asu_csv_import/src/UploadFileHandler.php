...
class UploadFileHandler {
   ...
   * @return array
   */
  public function validateImportData($file) {
    $errors = [];
   ...
   * @return array
   */
  public function createNodes($file){
    $field_definitions = $this->entityFieldManager->getFieldDefinitions('node', 'apartment');
    $update_nodes = [];
    $create_nodes = [];
   ...
   */
  public function getUploadedFile($fields){
   ...
   * @return array
   */
  private function getFieldTypes(array $header, $field_definitions){
    $field_types = [];
  ...
  public function createValue($data, $type){
    switch($type){
      case 'integer':
        return new Drupal\asu_csv_import\ImportTypes\NumberType($data);
        break;
      case 'string':
      case 'string_long':
        return new Drupal\asu_csv_import\ImportTypes\TextType($data);
        break;
      case 'link':
        return new Drupal\asu_csv_import\ImportTypes\LinkType($data);
        break;
      case 'decimal':
        return new Drupal\asu_csv_import\ImportTypes\DecimalType($data);
        break;
      case 'boolean':
        return new Drupal\asu_csv_import\ImportTypes\BooleanType($data);
        break;
      case 'datetime':
        return new Drupal\asu_csv_import\ImportTypes\DateType($data);
        break;
      default:
        return false;
      /*
