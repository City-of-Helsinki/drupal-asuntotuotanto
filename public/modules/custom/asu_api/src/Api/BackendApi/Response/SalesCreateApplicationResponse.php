 ...
 */
class SalesCreateApplicationResponse extends Response {
  ...
  public static function requestOk(ResponseInterface $response): bool {
    ...
    if ($code === 500) {
      throw new ApiException('Backend api error: ' . $response->getBody()->getContents(), $code);
    }
    if ($code >= 400 && $code <= 499) {
      throw new IllegalApplicationException($response->getBody()->getContents(), $code);
    }
    if ($code < 200 || $code > 299) {
      throw new RequestException('Bad status code: ' . $code);
    }
    return TRUE;
  }
